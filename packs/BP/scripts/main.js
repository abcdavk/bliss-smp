...
world.afterEvents.itemUse.subscribe((data) => {
    ...
    const powerSelectOff = offhandItem?.getDynamicProperty('bliss:power_selected');
    const powerSelectMain = itemStack.getDynamicProperty('bliss:power_selected');
            ...
            if (tickOff === 0 || tickMain === 0) {
                if (!player.isSneaking) {
                    if (powerSelectOff !== undefined) {
                        gem.onUse[powerSelectOff].executeItemUse(data);
                        cooldownOffhand?.startCooldown(player);
                    }
                    else if (powerSelectMain !== undefined) {
                        gem.onUse[powerSelectMain].executeItemUse(data);
